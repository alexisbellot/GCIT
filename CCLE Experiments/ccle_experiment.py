...
def load_ccle(drug_target='PLX4720', feature_type='both', normalize=False):
    ...
    if feature_type in ['expression', 'both']:
        ...
        expression = pd.read_csv('/home/alexis/Documents/Datasets/CCLE/expression.txt', delimiter='\t', header=2, index_col=1).iloc[:, 1:]
        expression.columns = [c.split(' (ACH')[0] for c in expression.columns]
        features = expression
    if feature_type in ['mutation', 'both']:
        # Load gene mutation
        mutations = pd.read_csv('/home/alexis/Documents/Datasets/CCLE/mutation.txt', delimiter='\t', header=2, index_col=1).iloc[:,1:]
    ...
    response = pd.read_csv('/home/alexis/Documents/Datasets/CCLE/response.csv', header=0, index_col=[0,2])
